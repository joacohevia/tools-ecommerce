...
function IconWhatsApp() {
  return (
    <svg className="w-4 h-4 flex-shrink-0 text-dark-muted group-hover:text-blue-400 transition-colors" viewBox="0 0 24 24" fill="currentColor">
...
/** Ícono SVG de teléfono. */
function IconTelefono() {
  return (
    <svg className="w-4 h-4 flex-shrink-0 text-dark-muted group-hover:text-blue-400 transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function IconReloj() {
  return (
    <svg className="w-4 h-4 flex-shrink-0 text-dark-muted" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export default function Footer() {
  return (
    <footer className="mt-4 bg-dark-blue border-t border-white/10">
                ...
                <li key={label}>
                  {href ? (
                    <a
                      ...
                      className="group flex items-start gap-2.5 text-dark-muted hover:text-blue-400 transition-colors"
                    >
                      <span className="mt-0.5"><Icono /></span>
                      <span className="text-sm">{valor}</span>
                    </a>
                  ) : (
                    <span className="group flex items-start gap-2.5 text-dark-muted">
                      <span className="mt-0.5"><Icono /></span>
                      <span className="text-sm">{valor}</span>
                    </span>
                  )}
            ...
            <ul className="flex flex-col gap-2">
              {NAV_LINKS.map(({ label, href }) => (
                <li key={label}>
                  <Link
                    to={href}
                    className="text-dark-muted hover:text-blue-400 transition-colors text-sm"
                  >
                    {label}
                  </Link>
                </li>
              ...
              {HORARIOS.map(({ dias, horario }) => (
                <li key={dias} className="flex items-start gap-2.5">
                  ...
                  <div className="flex flex-col">
                    <span className="text-dark-text text-sm font-medium">{dias}</span>
                    <span className="text-dark-muted text-sm">{horario}</span>
                  </div>
                </li>
              ))}
            </ul>
            ...
            <ul className="flex flex-col gap-3">
              {MEDIOS_PAGO.map(({ icono: Icono, texto }) => (
                <li key={texto} className="flex items-center gap-2.5">
                  <span><Icono /></span>
                  <span className="text-dark-muted text-sm">{texto}</span>
                </li>
              ))}
            </ul>
          </div>
